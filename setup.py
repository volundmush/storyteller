...
VERSION_PATH = os.path.join('storyteller', 'VERSION.txt')
...
def package_data():
    ...
    """
    file_set = []
    for root, dirs, files in os.walk('storyteller'):
        for f in files:
            if '.git' in f.split(os.path.normpath(os.path.join(root, f))):
                # Prevent the repo from being added.
                continue
            file_name = os.path.relpath(os.path.join(root, f), 'storyteller')
...
setup(
    ...
    description='Adds support for Storyteller games to Evennia/Athanor.',
